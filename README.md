...
![ModLoader](https://img.shields.io/badge/Mod%20Loader-Forge%20%7C%20NeoForge%20%7C%20Fabric-brightgreen)
...
**AFKStatus** is a lightweight, server-side mod for **Minecraft 1.21.10**, available for **Forge**, **NeoForge**, and **Fabric**. It automatically tracks player activity and marks users as AFK (Away From Keyboard) when they stop moving or chatting for a configurable period. Players can also manually toggle AFK status using a simple `/afk` command.
...
- **Play time pause (optional)**
  Stops the vanilla "Time Played" statistic from increasing while a player is marked AFK.
  Time before the player is marked AFK (`afk.triggerMinutes`) still counts, and scoreboard objectives using `minecraft.custom:minecraft.play_time` are paused too.
...
| `stats.pausePlayTimeWhileAfk`          | If true, "Time Played" stops increasing once a player is marked AFK      | `false`                                       | `true` / `false`          |
...
1. Install your preferred mod loader (**Forge**, **NeoForge**, or **Fabric** with Fabric API) on your Minecraft 1.21.10 server.
2. Place the matching **AFKStatus JAR** (`afkstatus-forge-*.jar`, `afkstatus-neoforge-*.jar`, or `afkstatus-fabric-*.jar`) into your server's `mods/` directory.
...
> - Thanks to the Forge, NeoForge, and Fabric communities for their tools and helpful examples.
